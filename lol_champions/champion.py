...
class Champion:
    ...
    def __str__(self) -> str:
        ...
        return (f"Champion Level: {self.level} Stats:\n"
                f"AD: {round(self.base_AD, 2)} HP: {round(self.base_HP, 2)} "
                f"AR: {round(self.base_AR, 2)} MR: {round(self.base_MR, 2)}")
    ...
    def add_stats(self, bonus_AD: float = 0.0, bonus_HP: float = 0.0, 
                  bonus_AR: float = 0.0, bonus_MR: float = 0.0):
