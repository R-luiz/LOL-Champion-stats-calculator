...
- **`lol_champions/champion.py`** — Base `Champion` dataclass: 5 core stats (AD, AP, HP, AR, MR) each with base/scaling/bonus/total fields, plus penetration stats (lethality, armor_pen_pct, magic_pen_flat, magic_pen_pct) and `is_melee` flag. `level_up()` applies scaling formula `base * (0.65 + 0.035 * level)` and grants a skill point. `add_stats()` adds bonus stats and pen from items/buffs.

- **`lol_champions/ability.py`** — `Ability` dataclass: tracks `name`, `max_level` (5 for Q/W/E, 3 for R), `current_level`.

- **`lol_champions/fiora.py`** — `Fiora(Champion)`: base stats, 4 `Ability` instances. Methods `Q()`, `W()`, `E()`, `passive()` return dicts with `raw_damage` and `damage_type` keys (compatible with `calculate_damage()`). `R()` has no `raw_damage` (damage comes from 4x passive procs). Ability stats stored as parallel lists indexed by `ability.current_level - 1`.
